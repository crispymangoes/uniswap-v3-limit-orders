...
contract LimitOrderRegistry is Owned, AutomationCompatibleInterface, ERC721Holder {
    ...
    // TODO add a check to make sure pool has been set up.
    function newOrder(
        UniswapV3Pool pool,
        int24 targetTick,
        uint96 amount,
    ...
    function performUpkeep(bytes calldata performData) external {
        (UniswapV3Pool pool, uint256[MAX_FILLS_PER_UPKEEP] memory ordersToFulfill) = abi.decode(
            performData,
            (UniswapV3Pool, uint256[10])
        );
